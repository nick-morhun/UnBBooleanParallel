 ...
 *
 * <br> <br> Typically, two 'Object3d' objects are created to apply boolean
 * operation. The methods splitFaces() and classifyFaces() are called in this
 * sequence for both objects, always using the other one as parameter. Then the
 * faces from both objects are collected according their status.
 *
...
public class Object3D implements Cloneable {
    ...
    ///**
    // * Tolerance.TOLerance value to test equalities
    // */
    //protected static final double Tolerance.TOL = 1e-10f;
    ...
    public Object3D(Solid solid) {
        ...
        for (int i = 0; i < verticesPoints.length; i++) {
            vertex = addVertex(verticesPoints[i], colors[i], VertexStatus.UNKNOWN);
    ...
    public Object3D(ArrayList<Vertex> vertices, ArrayList<Face> faces, Bound bound) {
    ...
    public Object clone() {
        try {
            ...
            clone.vertices = new ArrayList<Vertex>();
            for (int i = 0; i < vertices.size(); i++) {
                clone.vertices.add((Vertex) vertices.get(i).clone());
            }
            clone.faces = new ArrayList<Face>();
            for (int i = 0; i < vertices.size(); i++) {
     ...
     */
    protected Face addFace(Vertex v1, Vertex v2, Vertex v3) {
        if (!(v1.equals(v2) || v1.equals(v3) || v2.equals(v3))) {
            Face face = new Face(v1, v2, v3);
            if (face.getArea() > Tolerance.TOL) {
                faces.add(face);
                return face;
            } else {
                return null;
     ...
     * returned)
     */
    protected Vertex addVertex(Point3d pos, Color3f color, VertexStatus status) {
        int i;
    ...
    public void splitFaces(Object3D object) {
        ...
        for (int i = 0; i < this.getNumFaces(); i++) {
            face1 = this.getFace(i);
            //System.out.println("face " + i + " of " + faces.size());
            ...
            for (int j = 0; j < object.getNumFaces(); j++) {
                ...
                if (face1.getBound().overlap(face2.getBound())) {
                    ...
                    // POSSIBLE RESULTS: INTERSECT, NOT_INTERSECT,
                    // COPLANAR

                    // distance from the face1 vertices to the face2
                    // plane
                    distFace1Vert1 = this.computeDistance(face1.v1,
                            face2);
                    distFace1Vert2 = this.computeDistance(face1.v2,
                            face2);
                    distFace1Vert3 = this.computeDistance(face1.v3,
                            face2);

                    // distances signs from the face1 vertices to the
                    // face2 plane
                    ...
                    if (!(signFace1Vert1 == signFace1Vert2
                        ...
                        // distance from the face2 vertices to the face1
                        // plane
                        distFace2Vert1 = computeDistance(face2.v1,
                                face1);
                        distFace2Vert2 = computeDistance(face2.v2,
                                face1);
                        distFace2Vert3 = computeDistance(face2.v3,
                                face1);

                        // distances signs from the face2 vertices to
                        // the face1 plane
                        ...
                        if (!(signFace2Vert1 == signFace2Vert2
                            ...
                            // intersection of the face1 and the plane
                            // of face2
                            segment1 = new Segment(line, face1,
                                    signFace1Vert1, signFace1Vert2,
                                    signFace1Vert3);

                            // intersection of the face2 and the plane
                            // of face1
                            ...
                            if (segment1.intersect(segment2)) {
                                // PART II - SUBDIVIDING NON-COPLANAR
                                // POLYGONS

                                this.splitFace(i, segment1, segment2);

                                // prevent from infinite loop (with a
                                // loss of faces...)
                                ...
                                // if the face in the position isn't the
                                // same, there was a break
                                if (face1 != getFace(i)) {
                                    // if the generated solid is equal
                                    // the origin...
        ...
        // Print results.
        System.out.println("Object3D.splitFaces() v1.1 " + time + " ms.");
     ...
     */
    protected double computeDistance(Vertex vertex, Face face) {
     ...
     */
    protected void splitFace(int facePos, Segment segment1, Segment segment2) {
     ...
     */
    protected void breakFaceInTwo(int facePos, Point3d newPos, int splitEdge) {
     ...
     */
    protected void breakFaceInTwo(int facePos, Point3d newPos, Vertex endVertex) {
        Face face = faces.get(facePos);
        faces.remove(facePos);

        Vertex vertex = addVertex(newPos, face.v1.getColor(),
                VertexStatus.BOUNDARY);

        if (endVertex.equals(face.v1)) {
            this.addFace(face.v1, vertex, face.v3);
            this.addFace(vertex, face.v2, face.v3);
        } else if (endVertex.equals(face.v2)) {
            this.addFace(face.v2, vertex, face.v1);
            this.addFace(vertex, face.v3, face.v1);
        } else {
            this.addFace(face.v3, vertex, face.v2);
            this.addFace(vertex, face.v1, face.v2);
        }
    }
     ...
     */
    protected void breakFaceInThree(int facePos, Point3d newPos1,
            Point3d newPos2, int splitEdge) {
        Face face = (Face) faces.get(facePos);
        faces.remove(facePos);
     ...
     */
    protected void breakFaceInThree(int facePos, Point3d newPos, Vertex endVertex) {
        Face face = (Face) faces.get(facePos);
        faces.remove(facePos);
     ...
     * @param newPos new vertex position
     */
    protected void breakFaceInThree(int facePos, Point3d newPos) {
        Face face = (Face) faces.get(facePos);
        faces.remove(facePos);
     ...
     * @param endVertex vertex used for the split
     */
    protected void breakFaceInFour(int facePos, Point3d newPos1, Point3d newPos2,
            Vertex endVertex) {
        Face face = (Face) faces.get(facePos);
        faces.remove(facePos);
     ...
     * found
     */
    protected void breakFaceInFive(int facePos, Point3d newPos1, Point3d newPos2,
            int linedVertex) {
        Face face = (Face) faces.get(facePos);
        faces.remove(facePos);
    ...
    public void classifyFaces(Object3D object) {
        ...
        for (int i = 0; i < getNumFaces(); i++) {
            ...
            if (face.simpleClassify() == false) {
                ...
                // mark the vertices
                if (face.v1.getStatus() == VertexStatus.UNKNOWN) {
                    face.v1.mark(FaceStatus.toVertexStatus(face.getStatus()));
                }
                if (face.v2.getStatus() == VertexStatus.UNKNOWN) {
                    face.v2.mark(FaceStatus.toVertexStatus(face.getStatus()));
                }
                if (face.v3.getStatus() == VertexStatus.UNKNOWN) {
                    face.v3.mark(FaceStatus.toVertexStatus(face.getStatus()));
                }
            }
        }
        // Stop timing.
        time += System.currentTimeMillis();
        // Print results.
        System.out.println("classifyFaces() " + time + " ms");
    }
    ...
    public void invertInsideFaces() {
        ...
        for (int i = 0; i < getNumFaces(); i++) {
            face = getFace(i);
            if (face.getStatus() == FaceStatus.INSIDE) {
                face.invert();
            }
        }
    }
