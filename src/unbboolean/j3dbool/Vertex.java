...
public class Vertex implements Cloneable {
     ...
     */
    public Vertex(Point3d position, Color3f color, VertexStatus status) {
        this(position.x, position.y, position.z, color, status);
    }
     ...
     * @param status vertex status - UNKNOWN, BOUNDARY, INSIDE or OUTSIDE
     */
    public Vertex(double x, double y, double z, Color3f color, VertexStatus status) {
        this.color = (Color3f) color.clone();
    ...
    /**
     * Checks if an vertex is equal to another. To be equal, they have to have
     * the same coordinates(with some tolerance) and color
     *
     ...
     */
    public void mark(VertexStatus status) {
        //mark vertex
        this.status = status;
        ...
        for (int i = 0; i < adjacentVerts.length; i++) {
            if (adjacentVerts[i].getStatus() == VertexStatus.UNKNOWN) {
